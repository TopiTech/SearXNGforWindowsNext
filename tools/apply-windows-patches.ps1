            ...
            try:
                ...
                for res in addr_info:
                    ...
                    if ip_obj.is_global:
                        ...
                        # IPv6 must be wrapped in brackets for netloc
                        ip_str = f"[{ip_raw}]" if ip_obj.version == 6 else ip_raw
                        new_netloc = ip_str
                        if parsed.port:
                            new_netloc = f"{ip_str}:{parsed.port}"
                        return parsed._replace(netloc=new_netloc).geturl(), host
                raise RuntimeError(f'Could not find a global IP for {host}')
            except Exception as e:
                raise RuntimeError(f'DNS resolution failed for {host}: {e}')
        # SECURITY: TOCTOU mitigation — resolve once and use IP for all subsequent fetches in redirect chain
        current_url = request_url
        with httpx.Client(timeout=10.0, follow_redirects=False,
                          verify=verify_ssl, headers={'User-Agent': ua}) as client:

            for _ in range(5):
                safe_url, original_host = _get_safe_ip_url(current_url)
                # Inject original host into headers for virtual hosting
                # and SNI extension for HTTPS handshake success on IP-based requests.
                headers = {'User-Agent': ua, 'Host': original_host}
                extensions = {'sni_hostname': original_host}

                response = client.get(safe_url, headers=headers, extensions=extensions)
