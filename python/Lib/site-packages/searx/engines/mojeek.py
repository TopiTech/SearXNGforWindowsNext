...
def request(query, params):
    ...
    lang_value = traits.get_language(params["searxng_locale"], traits.custom.get("language_all"))
    region_value = traits.get_region(params["searxng_locale"], traits.custom.get("region_all"))

    cookies = {}
    # Always send the lb cookie.  Mojeek returns HTTP 500 when the lb cookie is
    # missing entirely or when it receives an empty lb value.  If the language
    # value is empty (e.g. traits not yet loaded), fall back to 'en'.
    cookies[language_param] = lang_value if lang_value else 'en'
    if region_value:
        cookies[region_param] = region_value
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    # The first option is usually "Default language" with an empty value.
    # If empty, use the first actual language to avoid sending an empty 'lb' cookie
    # which causes Mojeek to return HTTP 500.
    engine_traits.custom["language_all"] = languages[0] if languages[0] else (languages[1] if len(languages) > 1 else 'en')
    ...
    # `regions[0]` is usually "All Regions" with an empty value.
    # Skip it and use the first actual region; fall back to 'us' if empty.
    region_default = regions[1] if len(regions) > 1 else 'us'
    engine_traits.custom["region_all"] = region_default if region_default else (regions[2] if len(regions) > 2 else 'us')
